...
- [x] 21.3.2 Step 2 (Style & Design) Mobile Overhaul (`app/orders/new/page.tsx`):
  ...
  - **Docked Bottom Action Bar & Clearance**: Removed redundant navigation buttons from the scrollable container. Docked a sleek 2-button action row in the bottom confirmation bar on Step 2 (`fixed bottom-0 left-0 right-0 z-30 pb-safe`) with `[← واپس]` and `[اگلا مرحلہ: ناپ درج کریں (Next: Measurements) →]`, smoothly scrolling `#main-content` to top on advance, with `pb-44 pb-safe` bottom clearance.
