...
- [x] 19.9 Mobile Scroll Unlock, Viewport Physics Fix & Top Header Streamlining (`app/globals.css`, `components/layout/app-shell.tsx`, `lib/use-online-status.ts`):
  - **Viewport Touch Scroll Unlock (`app/globals.css`, `components/layout/app-shell.tsx`)**: Refactored `html, body` base styles in `app/globals.css` with `-webkit-overflow-scrolling: touch;`, `overflow-y: auto;`, `overflow-x: hidden;`, and configured `<main id="main-content">` in `AppShell` with `className={cn("flex-1 w-full overflow-y-auto overflow-x-hidden p-4 md:p-8 pb-safe overscroll-y-contain", navLayout === "drawer" ? "pb-8 md:pb-8" : "pb-36 md:pb-8")}` ensuring momentum scrolling without vertical gesture traps.
  - **Reactive Online Status Hook (`lib/use-online-status.ts`)**: Implemented SSR-safe `useOnlineStatus()` hook subscribing to `online`, `offline`, and `visibilitychange` events for real-time network tracking.
  - **Mobile Top Command Bar Streamlining (`components/layout/app-shell.tsx`)**: Refactored mobile top bar (`flex md:hidden items-center justify-between w-full h-14 min-h-14 px-3.5`) replacing the wide `<ConnectionPill />` badge with a sleek workshop title (`text-sm font-semibold text-white truncate max-w-[200px]`) and a compact pulsing status dot (`h-2 w-2 rounded-full animate-pulse`) glowing emerald when online and amber when offline, while preserving touch buttons for Search and Notifications, and keeping full desktop command bar intact.
  - **Automated Verification**: Verified 0 TypeScript compiler errors (`npx tsc --noEmit`), 159/159 database and repository test assertions pass (`scripts/verify_db.ts`), and clean production static export compiled into `out/` with all 28 static routes (`npm run build`).
...
- [x] 20.6 Cloud Build, Binary Compilation & Official Release v1.1.2-concept1 (`package.json`, `package-lock.json`, `gh release`):
  - Bumped version to `1.1.2` across `package.json` and `package-lock.json`.
  - Staged and committed snapshot `chore(release): v1.1.2-concept1 - viewport containment, mobile header clamping, universal pb-36 bottom clearance & rich onboarding visuals` and pushed to `main`.
  - Created annotated tag `v1.1.2-concept1` and pushed to remote triggering GitHub Actions CI/CD matrix build (`Run #33406622163`).
...
- [x] 20.9 Cloud Build, Binary Compilation & Official Release v1.1.4-concept1 (`package.json`, `package-lock.json`, `gh release`):
  - Bumped version to `1.1.4` across `package.json` and `package-lock.json`.
...
- [x] 22.3 Step 3: Compact Stitching Rates Grid & Purge Obsolete Navigation Radios (`app/settings/page.tsx`):
  * **Compact 2x2 Stitching Rates Grid (`mobileSection === 'rates'`)**: Replaced the loose 4-input fields with a strict 2x2 grid layout inside each garment card (Row 1 Pricing: Base Rate & Urgent Surcharge side-by-side; Row 2 Turnaround: Standard Delivery Days & Urgent Delivery Days side-by-side) with stacked bilingual Urdu/English labels and standardized `h-10 text-xs font-mono font-bold` inputs.
...
- [x] 23.3 Milestone 3: Non-Destructive Light Theme Layer, Semantic RGB Tokens & Capacitor Android Status Bar Sync:
...
- [x] 23.4 Milestone 4: WCAG High-Contrast Polish & Automated Playwright Audit:
...
- [x] 23.5 Milestone 5: 10/10 Premium Matte Atelier Light Theme & High-Contrast Bilingual Typography Polish:
